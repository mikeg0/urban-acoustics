...
this module. Two boto3 clients: one pinned to the internal endpoint for
head/create operations, and a signer pinned to the public endpoint so
``generate_presigned_url`` signs against the host the device will actually
connect to. A single client with URL-rewriting after signing would break
SigV4, which signs the ``host`` header.
"""
...
class Storage:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        sigv4_path = Config(signature_version="s3v4", s3={"addressing_style": "path"})
    ...
    @staticmethod
    def spectrogram_tile_key(device_id: UUID, hour_utc: datetime) -> str:
        """Key for a precomputed historical hour-tile PNG."""
        h = hour_utc.astimezone(timezone.utc) if hour_utc.tzinfo else hour_utc.replace(tzinfo=timezone.utc)
        return f"spectrograms/{h:%Y/%m/%d}/{device_id}/{h:%H}.png"
    ...
    def presign_get(self, key: str, *, ttl_seconds: int) -> PresignedGet:
        ...
        return PresignedGet(url=url, expires_at=_now_unix() + ttl_seconds)
    ...
    async def get_bytes(self, key: str) -> bytes | None:
        """Fetch object bytes, or None on 404. Used for server-side passthrough
        of small immutable assets (spectrogram tiles) where presigning would
        defeat browser caching."""

        def _get() -> bytes | None:
            try:
                resp = self._client.get_object(Bucket=self._settings.S3_BUCKET, Key=key)
    ...
    async def put_bytes(self, key: str, data: bytes, *, content_type: str) -> None:
        """Direct PUT of server-generated bytes. No presigning, no checksum
        header — caller owns the payload."""
