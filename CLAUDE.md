...
2. The **ingest worker** (`app/ingest/mqtt.py`, run as `python -m app.ingest.mqtt`) validates every payload against `app.contracts`, batches writes into Timescale, and emits Postgres `NOTIFY` for ephemeral data (spectrogram frames are **not** persisted — they fan out live and are dropped).
...
### Database
TimescaleDB (Postgres 16). Telemetry/health/spectrogram tables are hypertables with continuous aggregates for the `1m`/`1h` resolutions exposed by the read APIs. Migrations are Alembic (`backend/app/migrations/`, `script_location = app/migrations`) and run automatically via `alembic upgrade head` before uvicorn starts in the compose `command`. Internal storage is `TIMESTAMPTZ`; the API converts to Unix seconds at the wire boundary.
